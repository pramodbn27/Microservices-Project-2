...
"""
    ...
    node (label) {
        stage ('Checkout SCM'){
          git credentialsId: 'git', url: 'https://dptrealtime@bitbucket.org/dptrealtime/eos-order-api.git', branch: 'master'
          container('build') {
       ...
       stage ('Docker Build'){
          container('build') {
                stage('Build Image') {
                    ...
                    def customImage = docker.build("dpthub/eos-order-api:latest")
                    customImage.push()             
                    }
                }
            }
        ...
        stage ('Helm Chart') {
          container('build') {
            dir('charts') {
              ...
              sh '/usr/local/bin/helm package order-api'
              sh '/usr/local/bin/helm push-artifactory order-api-1.0.tgz https://eosartifact.jfrog.io/artifactory/eos-helm-local --username $username --password $password'
              }
            }
        }
        }
