...
"""
    ...
    node (label) {
        stage ('Checkout SCM'){
          git credentialsId: 'git', url: 'https://dptrealtime@bitbucket.org/dptrealtime/eos-kart-api.git', branch: 'master'
          container('build') {
       ...
       stage ('Docker Build'){
          container('build') {
                stage('Build Image') {
                    ...
                    def customImage = docker.build("dpthub/eos-kart-api:latest")
                    customImage.push()             
                    }
                }
            }
        ...
        stage ('Helm Chart') {
          container('build') {
            dir('charts') {
              ...
              sh '/usr/local/bin/helm package kart-api'
              sh '/usr/local/bin/helm push-artifactory kart-api-1.0.tgz https://eosartifact.jfrog.io/artifactory/eos-helm-local --username $username --password $password'
              }
            }
        }
        }
